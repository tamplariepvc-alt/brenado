...
async function uploadTaskPhoto(file) {
  ...
  const { data } = supabase.storage.from(TASK_PHOTOS_BUCKET).getPublicUrl(filePath);
...
function TaskForm({ onCreate, creating }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3 rounded-3xl bg-white p-4 shadow-sm">
      ...
      />
      <label className="block">
        <span className="mb-1 block text-sm font-medium text-slate-700">Poza din telefon</span>
        <input
          type="file"
          accept="image/*"
          capture="environment"
        ...
        />
        {photoName && <p className="mt-2 text-xs text-slate-500">Selectata: {photoName}</p>}
      </label>
      <button
        disabled={creating}
...
function TaskCard({ task, onUpdateStatus, onSaveDetails }) {
  ...
  return (
    <article className="rounded-3xl bg-white p-4 shadow-sm">
      ...
      {!(task.status === "Finalizata" && Array.isArray(task.final_photo_urls) && task.final_photo_urls.length > 0) && (
      ...
      {isEditing && (
        <div className="mt-4 space-y-3 rounded-2xl border border-slate-200 p-3">
          ...
          />
          <label className="block">
            <span className="mb-1 block text-sm font-medium text-slate-700">Schimba poza</span>
            <input
              type="file"
              accept="image/*"
              capture="environment"
              ...
              className="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none file:mr-3 file:rounded-xl file:border-0 file:bg-slate-100 file:px-3 file:py-2 file:text-sm file:font-medium"
            />
            {newPhotoName && <p className="mt-2 text-xs text-slate-500">Selectata: {newPhotoName}</p>}
          </label>
          <button
            type="button"
            onClick={handleSave}
  ...
  async function createTask(payload) {
    ...
    try {
      if (payload.photo_file) {
        uploadedPhotoUrl = await uploadTaskPhoto(payload.photo_file);
      }
    } catch (uploadError) {
      alert(uploadError.message || "Poza nu a putut fi incarcata.");
      console.error(uploadError);
      setCreating(false);
      return;
    }

    const { photo_file, ...taskPayload } = payload;
